...
module.exports = {
  ...
  legislativo_acuerdo:function (req, res, app, regreso){
    ...
    app.models[ "legislative_workpoint" ].find({ select: ['id_legislative_work_point'] }).exec(function createCB(err_autor, found ){
        // console.log("hola encontre",found);


          for (var i = found.length - 1; i >= 0; i--) {
            if (found[i].id_legislative_work_point) {
                control_trabajo[found[i].id_legislative_work_point]=1;
    ...
    var c = new crawler({
          ...
          callback : function (error, result, done) {
              ...
              $(elem).find(".tddatosazulrep, .tddatosnaranja").each(function(indextd, td) {
                ...
                switch (indextd) {
                  ...
                  case 2: //denominación
                      ...
                      iniciativa["id_legislative_work_point"]=unico;
                  break;
                  ...
                  case 6: //
                    ...
                    case 9: //Temas
                      ...
                      //  var themes_converted = only_text.replace(/1.-Diputados -/,'')
                      //                                   .replace(/1.-Senado -/,'')
                      //
                      //                                   .replace(/2.-Diputados -/,' ,')
                      //                                   .replace(/2.-Senado -/,' ,')
                      ...
                      //                                   .replace(/.-Para opinión/,'')
                      //                                   .replace(/Primera.-Para dictamen/,'')
                      //                                   .replace(/Segunda.-Para dictamen/,'');
                      //  var res = themes_converted.split(" ,");
                      //  themes.push(res);
                       iniciativa["work_themes"]=only_text;
                      break;
                    case 10: //Temas
                       var status = [];
                       var only_text = $(td).text();
            ...
            async.each(trabajo, function(inic, doneini){
              app.models[ "legislative_workpoint" ].create(inic).exec(function createCB(err, created){
                if(err!=null){
                 ...
                 async.each(created.author_id, function(autor, cb){
                    app.models[ "legislators" ].find({id_legislator_sil:autor}).populate("trabajopoint").exec(function createCB(err_autor, found ){
                      console.log(found,created,">---",iniini);
                       if (found[0]) {
                          found[0].trabajopoint.add(iniini);
